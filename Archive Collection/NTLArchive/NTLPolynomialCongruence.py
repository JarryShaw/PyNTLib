...
def cpsMCS(cgcExp, cgcCoe, modulo):
    (p, q) = primeFactorisation(modulo, wrap=True)      #分解模，以便判斷求解方式, 並生成因數表p與指數表q
...
def prmMCSLite(cgcExp, cgcCoe, mod, exp):
    tmpRmd = prmMCS(cgcExp, cgcCoe, mod)                    #獲取源素數模的同餘式的解
    if exp == 1:
        return tmpRmd
    
    remainder = prmMCSPro(cgcExp, cgcCoe, tmpRmd, mod, exp) #作高次同餘式的提升，求出源素數次冪模的同餘式的解
    return remainder
...
#     remainder = polynomialCongruence(cgcExp, cgcCoe, modulo)
