...
def cpsMCS(cgcExp, cgcCoe, modulo):
    fct = __import__('NTLPrimeFactorisation').primeFactorisation(modulo)    #分解模，以便判斷求解方式
...
def prmMCSPro(cgcExp, cgcCoe, rto, mod, exp):
    ...
    for tmpRto in rto:
        #尋找滿足(f'(x1),p)=1的x1
        if __import__('NTLGreatestCommonDivisor').greatestCommonDivisor(drv(tmpRto), mod) == 1:
            polyDrvMod = 0
            #用模重複平方法計算導式的值f'(x1) (mod p)
            for ptr in xrange(len(drvExp)):     
                polyDrvMod += __import__('NTLRepetiveSquareModulo').repetiveSquareModulo(drvCoe[ptr]*tmpRto, drvExp[ptr], mod)
