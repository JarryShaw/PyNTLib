...
def exponentPrimitiveRoot(base, exponent):
    primtiveRoot = []
    prrList = primePrimitiveRoot(base)

    for prr in prrList:
        if (prr**(base-1)) % base == 1 and GCD(base, ((prr**(base-1)) // base)) == 1:
            g = prr;        break
        if ((prr+base)**(base-1)) % base == 1 and GCD(base, (((prr+base)**(base-1)) // base)) == 1:
            g = prr+base;   break

    print g
    prc = primitiveResidueClass(eulerFunction(base**2))
    for num in prc:
        primtiveRoot.append(repetiveSquareModulo(g, num, int(base**exponent)))
...
if __name__ == '__main__':
    while True:
        try:
            base = int(raw_input('Base of the modulo is '))
            if base == 2 or not trivialDivision(base):
                print 'The base must be an odd prime number.'
                continue
        except ValueError:
            print 'Invalid input.'
            continue
    ...
    while True:
        try:
            exponent = int(raw_input('Exponent of the modulo is '))
            if exponent < 0:
                print 'The exponent must be possitive.'
                continue
        except ValueError:
            print 'Invalid input.'
            continue
    ...
    a = exponentPrimitiveRoot(base, exponent)
    print 'The primtive root(s) of modulo %d^%d is/are' %(base, exponent),
    for root in a:
        print root,
    print '.'